...
import TesterBugDetailsModal from "./TesterAssignedBugDetailsModal";
...
const TesterAssignedBugsList = () => {
  const [bugs, setBugs] = useState([]);
  const [selectedBug, setSelectedBug] = useState(null);
  const token = localStorage.getItem("token");

  // Fetch list of bugs assigned to the logged-in tester
  useEffect(() => {
    const fetchBugs = async () => {
      try {
        ...
        const res = await axios.get(
          `${process.env.REACT_APP_API_URL || "https://localhost:5000"}/api/bug-reports/assigned/tester/${encodeURIComponent(
            userEmail
          )}`,
          {
...
export default TesterAssignedBugsList;
