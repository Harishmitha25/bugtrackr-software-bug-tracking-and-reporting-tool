...
const AssignedBugDetails = ({ bugId, onClickingBack }) => {
  ...
  // Fetch bug details that is assigned to the team of the team lead
  useEffect(() => {
  ...
  //Fetch developers list of the team the team lead is the lead of
  useEffect(() => {
  ...
  //Fetch testers list of the team the team lead is the lead of
  useEffect(() => {
    if (bug?.application && bug?.assignedTeam) {
  ...
  const handleDuplicateCheck = async () => {
    try {
      ...
      //Filter out the current bug itself from the duplicates list
      const filteredDuplicates = duplicates.filter(
        (dup) => dup.bugId !== bug.bugId
      );
      ...
      if (filteredDuplicates.length > 0) {
        setBug((prev) => ({
          ...
          similarTo: filteredDuplicates.map((d) => d.bugId),
        }));

        toast.success("Duplicate check completed. Similar bugs found.");
        setSimilarBugs(filteredDuplicates);
      } else {
      ...
      <div className="flex gap-10">
        ...
        <div className="w-1/3 bg-gray-50 p-4 rounded-lg shadow-md">
          ...
          <TextField
            ...
            inputProps={{ maxLength: FIELD_LIMITS.statusReason.max }}
            fullWidth
            margin="normal"
            helperText={
              <span
              ...
              >
                {selectedReason.length < FIELD_LIMITS.statusReason.min
                  ...
                  : selectedReason.length >= FIELD_LIMITS.statusReason.max - 10
                  ? `Max limit: ${
                      FIELD_LIMITS.statusReason.max
                    } characters (Remaining - ${
                      (FIELD_LIMITS.statusReason.max - selectedReason.length, 0)
                    })`
                  : ""}
              </span>
            }
...
export default AssignedBugDetails;
