...
const BugReportFormAdmin = () => {
  ...
  const [teamChangeWarning, setTeamChangeWarning] = useState([]);
  ...
  //Based on the field data change update team and developers list
  const handleChange = async (e) => {
    ...
    if (name === "assignedToDeveloper" && value !== "Unassign") {
      try {
        const response = await axios.post(
          `${process.env.REACT_APP_API_URL || "https://localhost:5000"}/api/developers/check-availability`,
          {
    ...
    if (name === "application") {
      ...
      // To make sure that the developers list is retrieved properly if issueType was already selected before application
    ...
    if (name === "assignedTeam") {
      if (
        value !== "unassigned" &&
        teamAsscociatedToIssueType !== "unassigned" &&
        value !== teamAsscociatedToIssueType
      ) {
        setTeamChangeWarning(
          `The issue type "${bugReportFormData.issueType}" is best to be handled by the "${teamAsscociatedToIssueType}" team. You have selected "${value}".`
        );
      } else {
        setTeamChangeWarning("");
      }

      fetchDevelopers(bugReportFormData.application, value);
    }
  };
    ...
    if (teamName !== "unassigned") {
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_API_URL || "https://localhost:5000"}/api/developers/${appName}/${teamName}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
    ...
    <div className="flex flex-col h-full w-full">
      ...
      <div className="flex-1 overflow-auto p-6">
        <form onSubmit={handleSubmit} className="flex flex-col space-y-4">
          ...
          {/* Only visible if the application and issue type are selected*/}
          {bugReportFormData.application && bugReportFormData.issueType && (
            <FormControl
              fullWidth
              error={isSubmitted && bugReportFormErrors.assignedTeam}
            >
              <InputLabel>Assigned Team</InputLabel>
              <Select
                name="assignedTeam"
                value={bugReportFormData.assignedTeam || ""}
                onChange={handleChange}
                displayEmpty
              >
          ...
          {teamChangeWarning && (
            <p style={{ color: "text-warning" }}>{teamChangeWarning}</p>
          )}

          {/* Only visible if the developers list is greater than 0 meaning if we get the developers list for the team*/}
...
export default BugReportFormAdmin;
